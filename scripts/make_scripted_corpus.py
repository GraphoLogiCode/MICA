...
def provenance_pin(directory: str) -> dict | None:
    """The bank's provenance pin, when one exists. A pinned bank was generated by
    an OLDER generator than this code (the 2026-07-06 motion default change), and
    overwriting it would silently orphan every model trained on it — so main()
    refuses while the pin stands. Cascade A (the scheduled regeneration bundled
    with the next capture-batch retrain) removes the pin, or --unpin forces it."""
...
def main() -> int:
    ...
    if pin is not None and "--unpin" not in sys.argv:
        print("REFUSED: the banked corpus carries a provenance pin —")
